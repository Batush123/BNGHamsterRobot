...
int main()
{
	try
	{
		...
		Robot robot(hamster, &localizationManager, map.blownRadius, mapHeight, mapWidth);
			...
			try
			{
				...
				while (waypointIndex < numOfWaypoints)
				{
					...
					if (!isWaypointReached)
					{
						...
						//displayManager.PrintRouteCvMat(particles);
					}
					else
					{
						cout << endl <<
							"Reached waypoint on location - (" << hamsterWaypoint.x << ", " << hamsterWaypoint.y << ")" << endl;
					}
				...
				cout << "Robot has reached its destination!" << endl;
