...
{
	double hamsterStartX = startCol - (gridWidth/2);
	double hamsterStartY = startRow - (gridHeight/2);

	Location hamsterLocation =
	{
		.x = (waypoint.x - (gridWidth/2) - hamsterStartX),
		.y = -(waypoint.y - (gridHeight/2) - hamsterStartY)
	};
...
{
	Location currentWaypoint, hamsterWaypoint;

	for (int i = 0; i < numOfWaypoints; i++)
	{
		currentWaypoint = waypoints.at(i);
		hamsterWaypoint = ConvertToHamsterLocation(currentWaypoint);

		cout << "Current Waypoint is " << i << ": " <<
				"(" << hamsterWaypoint.x << ", " << hamsterWaypoint.y << ")" << "\t" <<
				"[The original is: " <<
				"(" << currentWaypoint.x << ", " << currentWaypoint.y << ")]" << endl;
	}
}
...
void DisplayManager::InitMapWithRoute()
{
	...
	mapFromPlannedRoute.resize(gridHeight);

	for (int i = 0; i < gridHeight; i++)
	{
		mapFromPlannedRoute.at(i).resize(gridWidth);
	}
	...
	else
	{
		...
		routeCvMat = cv::Mat(gridHeight, gridWidth, CV_8UC3, cv::Scalar::all(0));
